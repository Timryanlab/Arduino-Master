...
//
// Change_log
// 2.3 Altered 2.2 to allow triggering on superres scope
...
int ins[] = {13,11}; // {Camera, Ext. Shut}
int outs[] = {7, 4, 2, 3}; // {Stimulate, Laser1, Laser2, ...}
int ext_trig = 10;
int shut = 12;
...
void loop() {
  ...
  if(Serial.available() >0){ // if there is a usb input, execute following
    ...
    switch(cmd){ // switch statement allows for building of a variety of command interfaces
      ...
      case 52: // If the incoming bit is '4' then toggle 405 shutter
        l405 = (l405+1)%2;
        if(l405 == 1){
        ...
        break;
                                                                                                // variables and commands used
      case 78: // if incoming bit is 'N' change number of stimuli                               // 55: 4: Toggle 405 shutter
      ...
      case 83: // If the incoming bit is 'S' then stimulate now                                 // 105: i: toggle inverter
        flag = 1; // Run a train of stimuli at p intervals                                       // 107: k: toggle simultaneous camera acquisition
        Serial.print("STIM!\n"); // communicate sitmulation                                     // 114: r: reset frame number
        break;                                                                                  // 115: s: force train of stimulation 
                                                                                                // 116: t: force external trigger
      case 97: // If incoming byte is 'a' then arm stimulation                                  // 119: w: toggle switcher
        arm = (arm +1) % 2;                                                                     //
      ...
      case 116: // If incoming byte is 't' the following bytes will be a framenumber
        take_frame();
        Serial.print("Sent External Trigger "); // Confirm input
        Serial.print("\n"); // newline
        break;
